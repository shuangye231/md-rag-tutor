  ...
  return (
    <article className={`message-row ${isUser ? 'user-message' : 'assistant-message'}`}>
      ...
      <div className="message-content">
        {!isUser && <div className="assistant-meta"><strong className="assistant-name">全能 AI 导师</strong><button type="button" className="message-voice-button" onClick={toggleSpeech} aria-label={speaking ? '停止朗读' : '朗读回答'} title={speaking ? '停止朗读' : '朗读回答'}>{speaking ? <Square size={13} /> : <Volume2 size={15} />}</button></div>}
        {message.showcase ? (
          <AnswerCard />
        ) : (
