import { ArrowUp, Check, ChevronDown, FileText, FolderKanban, GraduationCap, ListTodo, MessageCircle, Mic, Paperclip, Square, X } from 'lucide-react'
import { useEffect, useId, useRef, useState, type ChangeEvent, type KeyboardEvent } from 'react'
import { createPortal } from 'react-dom'
import type { AppMode, ModelOption, SessionAttachment } from '../types'
    ...
    <div className="composer-wrap">
      <div className={`composer ${loading ? 'is-loading' : ''}`}>
        <textarea
          ...
          onKeyDown={onKeyDown}
          placeholder={appMode === 'task' ? (workspaceSelected ? '描述要在当前工作区中完成的任务...' : '请先选择任务工作区') : '继续提问，或粘贴一段内容让 AI 帮你讲解...'}
          rows={2}
          disabled={loading}
          aria-label="输入问题"
        />
        ...
        <div className="composer-mode-row">
          <div className="composer-mode" role="group" aria-label="对话模式">
            <button type="button" className={appMode === 'chat' ? 'is-active' : ''} aria-pressed={appMode === 'chat'} onClick={() => onAppModeChange('chat')}><MessageCircle size={14} />聊天</button>
            <button type="button" className={appMode === 'task' ? 'is-active' : ''} aria-pressed={appMode === 'task'} onClick={() => onAppModeChange('task')}><ListTodo size={14} />任务</button>
          </div>
          {appMode === 'task' && <button className={`task-workspace-chip ${workspaceSelected ? 'is-selected' : ''}`} type="button" onClick={onWorkspace} title="打开任务工作区"><FolderKanban size={14} /><span>{workspaceSelected ? workspaceName : '选择任务工作区'}</span></button>}
        </div>
