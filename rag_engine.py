...
_llm_client = None  # 持久的客户端，避免重复创建连接
_llm_client_config = None  # 记录客户端对应的配置，配置变了就重建


def get_llm():
    """获取当前模型的 OpenAI 客户端 + model_name
    复用客户端：避免每次创建新 httpx 连接导致 502
    """
    global _llm_client, _llm_client_config
    cfg = MODEL_CONFIG[current_model_id]
    if current_model_id in ("flash", "pro"):
        api_key = os.getenv("CCODE_API_KEY", "") or os.getenv("PRO_API_KEY", "")
    else:
        api_key = cfg["api_key"]

    # 如果客户端不存在或配置变了，重新创建
    config_key = (api_key, cfg["base_url"])
...
# ── AI 学习导师 系统提示词 ──────────────────────────────
SYSTEM_PROMPT = """你是“全能 AI 导师”，由产品团队开发和维护，专门用于学习辅导。你的使命是帮助**零基础小白**系统学习人工智能领域的知识，提供清晰的职业规划和学习路线。

## 产品身份与保密
- 对外身份始终是“全能 AI 导师”，不要自称任何底层模型或模型厂商。
- 用户询问模型、厂商、API、训练方或供应商时，不得提及任何具体底层模型、公司或接口名称。
- 统一说明：我是全能 AI 导师，由产品团队开发和维护，专门用于学习辅导。底层技术与供应链属于内部实现，不对外披露，不影响你的使用。
- 不要声称底层大模型完全由产品团队从零训练。

## 核心原则
1. 通俗易懂 — 用生活化的比喻解释复杂概念，避免术语轰炸。遇到专业名词（RAG、Fine-tuning、Embedding、LoRA、Transformer、Agent、MCP、RHLF等）必须主动解释。
...
## 你精通的知识领域
1. AI应用/AI工具使用（ChatGPT、Claude、AI绘画、AI视频、AI办公提效、提示词工程）
2. AI产品经理（需求定义、模型选型、AI产品落地全流程）
3. AI Agent开发（LangChain、LangGraph、AutoGen、CrewAI、Dify、MCP、Function Calling）
4. AI工程师/AI开发（Python、机器学习、深度学习、LLM原理、RAG、模型微调、模型部署、API开发）
5. AI数据/算法（数据处理、特征工程、模型训练与评估）
6. AI行业就业与职业规划（各方向薪资、转行路径、面试指导）
7. AI前沿动态（最新模型、行业趋势、论文解读）
    ...
    def ask(self, query: str, history: list = None) -> str:
        ...
        for retry in range(3):
            try:
                ...
                last_err = e
                print(f"⚠️ 回答模型调用失败({retry+1}/3): {e}")
                if retry < 2:
                    time.sleep(3)
...
class UnifiedRagAgent:
    ...
    def ask(self, query: str, history: list = None, learning_context: str = "", tutor_mode: str = "explain") -> str:
        """
        统一问答入口：AI 自己决定是否检索知识库
        """
        # 1. 先让 AI 判断是否需要检索（加重试，Ollama 首次加载可能 502）
        import time
        llm_judge, judge_model = get_llm()
        judge_prompt = f"""你是一个AI知识库管理员。你的任务就是判断"是否需要查资料"来回答用户的问题。

规则：
- 如果问题涉及**具体知识点、术语解释、概念对比**（例如：什么是LLM？Transformer和RNN的区别？），回答 "need_search"
- 如果问题是**闲聊、创作、开放性问题**（例如：帮我写首诗、你好吗、你有什么建议），回答 "no_search"
...
用户问题：{query}"""
        ...
        for retry in range(3):
            try:
                ...
                print(f"⚠️ 判断模型调用失败({retry+1}/3): {e}")
                time.sleep(2)
