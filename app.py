...
BRAND_IDENTITY_ANSWER = (
    "我是全能 AI 导师，由产品团队开发和维护，专门用于学习辅导。"
...
def create_user_session(username: str) -> str:
    ...
    conn = sqlite3.connect(str(DB_PATH))
    conn.execute("DELETE FROM user_sessions WHERE expires_at < ?", (datetime.now().isoformat(),))
    conn.execute("INSERT INTO user_sessions (token_hash, username, expires_at, created_at) VALUES (?, ?, ?, ?)", (hash_api_key(token), username[:40], (datetime.now() + timedelta(days=30)).isoformat(), datetime.now().isoformat()))
    conn.commit(); conn.close()
    return token
...
def require_authenticated_user(username: str, session_token: str) -> str:
    ...
    conn = sqlite3.connect(str(DB_PATH))
    row = conn.execute("SELECT username, expires_at FROM user_sessions WHERE token_hash = ?", (hash_api_key(session_token),)).fetchone()
    conn.close()
    if not row or row[0] != username[:40] or row[1] < datetime.now().isoformat():
        raise HTTPException(401, "登录已失效，请重新登录")
    return row[0]
...
# ── 初始化 ───────────────────────────────────────────────
app = FastAPI(title="AI 全能导师", version="4.1.0")
...
async def startup():
    ...
    for path in sorted(UPLOAD_DIR.glob("*.md")):
        try:
            tutor.index_document(path.read_text(encoding="utf-8"), filename=path.name)
        except (OSError, UnicodeError) as error:
            print(f"⚠️ 跳过无法读取的补充知识文件 {path.name}: {error}")
    print("🚀 全能 AI 导师 v4.1 启动完成！")
...
async def login(data: dict):
    ...
    return {"status": "ok", "message": "登录成功", "username": username, "session_token": create_user_session(username)}
...
async def change_password(data: dict, x_session_token: str = Header(default="")):
    ...
    if not username or not old_password or len(new_password) < 4:
        raise HTTPException(400, "请填写完整信息，新密码至少 4 位")
...
async def ask_question(data: dict, x_api_key: str = Header(default=""), x_session_token: str = Header(default="")):
    ...
    if session_id not in sessions:
        sessions[session_id] = []
        session_stats[session_id] = {"count": 0, "model": "", "tokens": 0}
    ...
    if attachment_context:
        model_query = f"请结合当前会话附件回答。\n\n{attachment_context}\n\n【用户问题】\n{query}"

    # 统计
    session_stats[session_id]["count"] += 1
    if username == "anonymous":
        total_global_count += 1
    session_stats[session_id]["model"] = MODEL_CONFIG[rag_engine.current_model_id]["name"]

    # 估算 token 数（中文约 1 字 2 token，英文约 1 词 1 token）
    input_tokens = len(query) * 2

    try:
        memory = get_learning_memory(username) if username != "anonymous" else {"active_topics": [], "weak_points": [], "mastery": {}}
        answer = tutor.ask(model_query, history=history, learning_context=learning_context(memory), tutor_mode=tutor_mode)
    ...
    session_stats[session_id]["tokens"] += total_tokens
...
{source}"""
    try:
        ...
        response = client.chat.completions.create(model=model, messages=[{"role": "user", "content": prompt}], temperature=0.35)
...
@app.post("/api/upload")
async def upload_file(file: UploadFile = File(...), username: str = Form(default=""), x_session_token: str = Header(default="")):
    ...
    result = tutor.index_document(content, filename=save_name)

    conn = sqlite3.connect(str(DB_PATH))
    conn.execute(
        "INSERT OR REPLACE INTO knowledge_uploads (stored_name, original_name, username, chunks, created_at) VALUES (?, ?, ?, ?, ?)",
        (save_name, Path(file.filename).name, owner, result["chunks"], datetime.now().isoformat()),
    )
    conn.commit()
    conn.close()
    ...
    return {
        ...
        "message": f"已补充 {result['chunks']} 个知识片段"
    }
...
async def list_knowledge_files(username: str, x_session_token: str = Header(default="")):
    username = require_path_user(username, x_session_token)
    conn = sqlite3.connect(str(DB_PATH))
    rows = conn.execute("SELECT stored_name, original_name, username, chunks FROM knowledge_uploads").fetchall()
    conn.close()
    metadata = {row[0]: row[1:] for row in rows}
    files = sorted(UPLOAD_DIR.glob("*.md"), key=lambda item: item.stat().st_mtime, reverse=True)
    return {"files": [knowledge_file_info(path, metadata.get(path.name)) for path in files], "username": username}
...
async def delete_knowledge_file(username: str, stored_name: str, x_session_token: str = Header(default="")):
    require_path_user(username, x_session_token)
    safe_name = Path(stored_name).name
    if safe_name != stored_name or not safe_name.lower().endswith(".md"):
    ...
    conn = sqlite3.connect(str(DB_PATH))
    conn.execute("DELETE FROM knowledge_uploads WHERE stored_name = ?", (safe_name,))
    conn.commit()
    conn.close()
    return {"status": "ok"}
...
async def add_session_attachment(session_id: str = Form(...), file: UploadFile = File(...)):
    raw = await file.read()
    if not raw:
    ...
    items = session_attachments.setdefault(session_id[:80], [])
    if len(items) >= 5:
...
@app.delete("/api/session/attachment/{session_id}/{attachment_id}")
async def remove_session_attachment(session_id: str, attachment_id: str):
    items = session_attachments.get(session_id[:80], [])
    session_attachments[session_id[:80]] = [item for item in items if item["id"] != attachment_id]
    return {"status": "ok"}
...
async def switch_model_api(data: dict):
    ...
    pro_api_key = data.get("pro_api_key", "")
    result = switch_model(model_id, pro_api_key)
...
async def get_session_stats(data: dict):
    session_id = data.get("session_id", "default")
    s = session_stats.get(session_id, {"count": 0, "model": "", "tokens": 0})
    s["model"] = MODEL_CONFIG[rag_engine.current_model_id]["name"]
    return s
...
if __name__ == "__main__":
    ...
    print(f"🚀 AI 全能导师 v4.1 启动成功！")
    print(f"📖 打开浏览器访问: http://localhost:{port}")
    print(f"📊 新增统计功能：今日提问、总提问、Token 用量")
